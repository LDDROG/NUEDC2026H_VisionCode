...
#define SPI_DMA_CHANNEL_NONE 0xFFU

/* STM32 HAL compatible type aliases for API stability */
typedef SPI_Regs SPI_HandleTypeDef;
...
typedef struct spi_ins_temp
{
    ...
    uint32_t cs_pin;               // 片选信号对应的引脚mask,如DL_GPIO_PIN_1,DL_GPIO_PIN_22等等

    SPI_TXRX_MODE_e spi_work_mode; // 传输工作模式
    uint8_t CS_State;              // 片选信号状态,用于中断模式下的片选控制
    uint8_t *cs_pin_state;         // 片选信号状态,用于中断模式下的片选控制

    uint8_t dma_rx_channel;        // DMA接收通道, SPI_DMA_CHANNEL_NONE表示不使用DMA
    uint8_t dma_tx_channel;        // DMA发送通道, SPI_DMA_CHANNEL_NONE表示不使用DMA
    ...
    uint8_t transfer_mode;         // 运行时状态机(内部使用)
