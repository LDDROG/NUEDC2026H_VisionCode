...
typedef I2C_Regs I2C_HandleTypeDef;

#define IIC_DEVICE_CNT 1   // MC02只有1个I2C接口可用，
#define MX_IIC_SLAVE_CNT 8 // 最大从机数目,根据需要修改
#define IIC_DMA_CHANNEL_NONE ((uint8_t) 0xFF)
...
typedef struct iic_temp_s
{
    ...
    uint16_t tx_count;                     // 已发送计数
    uint8_t *rx_buffer;                    // 接收缓冲区指针
    uint16_t rx_len;                       // 接收长度
    uint16_t rx_count;                     // 已接收计数
    uint8_t dma_channel;                   // DMA通道, 收发分时复用
    bool transfer_busy;                    // 传输进行标志
    bool use_dma;                          // 当前传输是否走DMA
    IIC_Seq_Mode_e seq_mode;               // 当前序列传输模式
...
typedef struct
{
    ...
    uint8_t dma_channel;             // DMA通道号,未使用可填IIC_DMA_CHANNEL_NONE
    IIC_Work_Mode_e work_mode;       // 工作模式
...
/**
 * @brief I2C中断分发函数，建议在I2C0/I2C1 IRQHandler中调用
 *
 * @param hi2c I2C外设基地址
 */
